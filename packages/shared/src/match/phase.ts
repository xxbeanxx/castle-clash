...
/** `Draft` is a pass-through stub until Phase 7 wires an actual power-up
 *  pick — `advanceMatchPhase` moves straight through it into `Countdown`. */
...
export function advanceMatchPhase(state: MatchPhaseState, input: MatchPhaseInput): MatchPhaseResult {
  ...
  switch (state.phase) {
    ...
    case "Draft": {
      // Pass-through stub until Phase 7 lands a real power-up pick.
      if (input.playerCount < MIN_PLAYERS) {
        return { state: withPhase(state, { phase: "Waiting" }), events };
      }
      return {

...
**Update (see `docs/research/toolchain-ts7-oxc-migration.md`):** ESLint/`typescript-eslint` were replaced by oxlint, which removes the `typescript-eslint` peer-range blocker above. The `tsup` `.d.ts`-bundling blocker is independent of that and still applies — re-checked live and still open (`egoist/tsup#1408`) as of that doc's research date. `typescript` stays pinned to `6.0.3` until both a stable `7.1+` ships and `tsup` (or an equivalent dts step) supports it; that doc is the current source of truth for this pin, not this paragraph.
...
**(b) Was the plan correct?** Unverified for the official `docker.io/library/node:24-alpine` image at the time this entry was first written (no Docker available). Later in the same session, `podman` turned out to be installed after all (just not checked at first — the original "Docker isn't installed" note in the handoff doc didn't mention podman specifically), and both containerfiles were actually built and run with it. The containerfiles' own `RUN npm install -g pnpm@12.4.1 turbo@2.10.13` step was never compared against a `corepack enable` alternative inside the same base image, so whether `node:24-alpine` itself ships corepack remains genuinely unverified — but the `npm install -g` approach is now confirmed to work end-to-end (both images build, and the server/client containers run correctly), which is what actually matters.

**(c) Action taken:** Both `apps/server/containerfile` and `apps/client/containerfile` (renamed from `Dockerfile` partway through the session — the user builds with `podman`) install pnpm and turbo via `npm install -g pnpm@12.4.1 turbo@2.10.13` instead of `corepack enable`, with an inline comment explaining why. This is a deviation from the plan's literal Dockerfile step, not just a version bump — flagged here for the same reason the other corrections are: so it's a recorded, deliberate choice rather than a silent one.
...
- This session's own `podman build`/`podman run` of both containerfiles, end to end (see the "fix: real podman build/run bugs in both containerfiles" commit for the three bugs that surfaced doing so — none of them corepack-related, but it's the reason this entry's status could be upgraded from "dry-run only" to "actually built and ran").

...
- SPA mode is alive and documented exactly as the plan describes: set `ssr: false` in `react-router.config.ts`. The framework still server-renders the root route once at *build time* to produce `index.html`, then the app hydrates and all further navigation is client-side. Source: https://reactrouter.com/how-to/spa
